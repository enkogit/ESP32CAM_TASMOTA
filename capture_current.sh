...
if [ $ps -le $mn ]; then
  curl "http://192.168.1.XXX/control?var=framesize&val=10"
  curl "http://192.168.1.XXX/control?var=raw_gma&val=0"
  # curl "http://192.168.1.XXX/control?var=hmirror&val=1" # Adjust horizontal and verital flip
  # curl "http://192.168.1.XXX/control?var=vflip&val=1"
  curl "http://192.168.1.XXX/control?var=dcw&val=0"
  sleep 5.0
  wget http://192.168.1.XXX/capture -O /home/pi/Timelapse/tl_weather_`date +%Y-%m-%d_%H-%M-%S`.jpg
  latest=$(ls /home/pi/Timelapse/tl_weather_*.jpg -rt1 | tail -1)
fi
